...
if(!$_POST['q']||!$_POST['w']||!$_POST['r']):
	...
	$list = '<div class="row"><div class="col-md-12"><div class="alert alert-success"><b>'.$json->resultnum.'</b> artists found associated with"<b>'.trim($_POST['q']).'</b>"</div></div>';
	...
	for($i=0;$i<$res;$i++):
		if(count($json->result)<1)
			...
			$list .= '<div class="col-md-3"><img class="img-circle" src="'.$json->result[$i]->bigpic.'" width="128" height="128"><p><a class="btn btn-default" href="singer.php?id='.$json->result[$i]->id.'" role="button">'.$singer.' &raquo;</a></p></div>';
		}
	endfor;
	$list .= '</div>';
